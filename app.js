...
function getManualContent() {
    ...
    `;
}// Agregar estas variables al inicio del archivo app.js
let calculatorValue = '0';
let calculatorHistory = '';
let calculatorLastOperation = null;
let calculatorResetInput = false;

// Agregar estas funciones al archivo app.js
function calculatorInput(value) {
    if (calculatorResetInput) {
        calculatorValue = '0';
        calculatorResetInput = false;
    }
...
function calculatorOperation(op) {
    if (calculatorLastOperation && !calculatorResetInput) {
        calculatorCalculate();
    }
    
    calculatorHistory = calculatorValue + ' ' + op;
    calculatorLastOperation = op;
    calculatorResetInput = true;
    updateCalculatorDisplay();
}

function calculatorCalculate() {
    if (!calculatorLastOperation) return;
    
    const num1 = parseFloat(calculatorHistory);
    const num2 = parseFloat(calculatorValue);
    let result;
    
    switch (calculatorLastOperation) {
        case '+':
            result = num1 + num2;
            break;
        case '-':
...
function updateCalculatorDisplay() {
    ...
    if (inputElement) inputElement.textContent = calculatorValue;
    if (historyElement) historyElement.textContent = calculatorHistory;
}

// Modificar la función showSection para incluir la calculadora
function showSection(sectionId) {
    try {
        switch(sectionId) {
            case 'claims':
...
// Modificar la función goBack para incluir la calculadora
function goBack() {
    try {
        if (document.getElementById('terms-screen')?.classList.contains('active') ||
            document.getElementById('privacy-screen')?.classList.contains('active') ||
            ...
            showScreen('main-screen');
        } else if (document.getElementById('calculator-screen')?.classList.contains('active')) {
            showScreen('main-screen');
        }
    } catch (error) {
        console.error('Error al volver:', error);
